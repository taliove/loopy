...
function Edge(model, config){
	...
	self.draw = function(ctx){
		...
		// Check if the arc radius is too large compared to the distance between nodes
		// This happens when nodes are very close together
		var nodeDistance = w / 2; // un-retina (w is already retina * 2)
		
		// Don't draw if nodes are too close (less than 1.5 times the sum of their radii)
		var minDistance = (self.from.radius + self.to.radius) * 1.5;
		var shouldDrawArc = (nodeDistance > minDistance);
